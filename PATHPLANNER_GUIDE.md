...
**RobotControl2026 — FRC 2026 賽季****RobotControl2026 — FRC 2026 賽季**
...
2. [機器人設定參數](#機器人設定參數)2. [機器人設定參數](#機器人設定參數)

3. [可用的 Named Commands（命名指令）](#可用的-named-commands)3. [可用的 Named Commands（命名指令）](#可用的-named-commands)

4. [核心技術：邊走邊射 (Shoot on the Move)](#核心技術邊走邊射-shoot-on-the-move)4. [在 PathPlanner GUI 中建立自動程式](#在-pathplanner-gui-中建立自動程式)

5. [在 PathPlanner GUI 中建立自動程式](#在-pathplanner-gui-中建立自動程式)5. [Auto 結構類型](#auto-結構類型)
...
11. [建立新路徑的步驟](#建立新路徑的步驟)11. [常見問題與注意事項](#常見問題與注意事項)

12. [常見問題與注意事項](#常見問題與注意事項)
...
- **路徑 (.path)**：機器人行駛的軌跡（XY 平移 + 旋轉）- **Auto (.auto)**：包含路徑與命令的完整自主期程序

- **Auto (.auto)**：包含路徑與命令的完整自主期程序- **Named Command**：在 Java 中預先定義好的指令，在 PathPlanner GUI 中以「名稱字串」呼叫
...
| 機器人寬度 | 0.845 m || 機器人長度 | 0.845 m |

| 機器人長度 | 0.845 m || 驅動模式 | Holonomic（全向） |

| 驅動模式 | Holonomic（全向） || 預設最大速度 | 1.5 m/s |

| 預設最大速度 | 1.5 m/s || 預設最大加速度 | 1.5 m/s² |

| 預設最大加速度 | 1.5 m/s² || 最大角速度 | 25.0 deg/s |

| 最大角速度 | 25.0 deg/s || 驅動馬達 | Kraken X60 |

| 驅動馬達 | Kraken X60 || 最高馬達速度 | 5.3 m/s |
...
**`Auto Aim Shoot` 的完整流程：**| `transport wait shoot` | 等速後出球 | 無超時 | 等射手達到目標 RPS 後才啟動輸送帶（需搭配 `shoot work` 使用） |

1. 覆寫 PathPlanner 的旋轉控制 → 自動朝向 Hub 旋轉

2. 依據距離計算射手目標 RPS（多項式查表）> **推薦使用 `Auto Shoot`**，它包含完整的射擊流程並有 4 秒超時保護。

3. 角度對齊（≤ 2°）+ 射手達速（誤差 < 3 RPS）→ 啟動 Transport + Up-to-shoot 送球

4. 送球 0.8 秒後自動結束，或總超時 4 秒強制結束### 🟡 吸球指令

5. 結束時歸還旋轉控制權給 PathPlanner
...
### 🟡 吸球指令| `Start Intake` | 僅啟動吸球滾輪 | 無超時 | 只啟動滾輪，不含輸送帶，需另外停止 |
...
- 離開吸球區**後**觸發 `Stop Intake`### 步驟一：建立或選擇路徑
...
## 核心技術：邊走邊射 (Shoot on the Move)4. 設定路徑的起點、終點與中間控制點

5. 設定 **globalConstraints**（速度/加速度）

### 原理6. 設定 **goalEndState**（到達終點時的方向）
...
- 底盤完全交由 PathPlanner 控制，平移與旋轉完美融合- 從下拉選單選擇已建立的路徑名稱

- 不會搶奪控制權導致機器人停下或抽搐- 路徑可串聯，也可平行執行
...
| 旋轉控制 | `setAimSpeed()` 疊加 | `setRotationTargetOverride()` 覆寫 |4. 設定是否需要超時（Named Command 本身的超時，獨立於 Java 中的超時）
...
2. 連接到本專案的 `src/main/deploy/pathplanner/` 目錄用途：射擊後再移動，確保先完成射擊再走
...
6. 設定 **goalEndState**（到達終點時的速度和方向）### 2. 平行執行（Parallel Group）
...
1. 點擊左側 **Autos** → **新增 Auto**┌─ [路徑：去吃球] ─────────┐

2. 點擊畫面右側的 **+** 按鈕，加入各項元素│                           │ → 都完成後繼續

└─ [Auto Intake：吸球] ────┘

### 步驟三：加入 Named Command```



1. 點擊 **+** 按鈕用途：邊移動邊吸球，節省時間

2. 選擇 **Named Command**

3. 在名稱欄輸入指令名稱（**必須與下表完全一致**）：> ⚠️ 平行群組中，**路徑跟隨與 Named Command 同時進行**。

   - `Auto Aim Shoot` — 邊走邊瞄準射擊> 通常路徑的行駛時間 > 指令時間（2.5s），讓吸球在移動中自然完成。
